...
-- PK: cliente_id (identificador único do cliente)
-- ----------------------------------------------------------------
...
CREATE TABLE operacoes (
    operacao_id      VARCHAR(20)  PRIMARY KEY,
...
-- PK: (cliente_id, data_referencia) | FK: cliente_id → clientes
-- ----------------------------------------------------------------
...
-- PK: (cliente_id, tipo_limite) | FK: cliente_id → clientes
-- ----------------------------------------------------------------
...
--     (colunas, tipos, PKs, FKs, constraints)
-- ----------------------------------------------------------------
EXEC sp_help 'clientes';
EXEC sp_help 'operacoes';
EXEC sp_help 'ratings';
EXEC sp_help 'limites';
EXEC sp_help 'exposicoes';
...
-- ================================================================
-- SEÇÃO 3 — QUERIES ANALÍTICAS DO CASE
-- ================================================================
...
distribuicao AS (
    SELECT segmento, COUNT(*) AS n_clientes,
        ...
        CAST(ROUND(STDEV(exposicao_total)/1000000,2) AS DECIMAL(15,2)) AS desvio_MM,
        CAST(ROUND(STDEV(exposicao_total)/NULLIF(AVG(exposicao_total),0)*100,1)
